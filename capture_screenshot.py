...
def build_events_for_timestamp(host, project_id, api_key, session_id, key_timestamp_ms):
    ...
    for source in manifest:
        ...
        end_ts = source.get("end_timestamp")
        if end_ts and key_timestamp_ms and parse_iso_to_ms(end_ts) >= key_timestamp_ms:
            break
    return sorted(events, key=lambda e: e.get("timestamp", 0))


def render_and_screenshot(events, key_timestamp_ms, out_path, vendor_dir):
    html_path = os.path.join(vendor_dir, "_capture_shell.html")
    offset_ms = max(0, key_timestamp_ms - events[0].get("timestamp", key_timestamp_ms))
...
window.__player.goto({offset_ms}, false);
window.__ready = true;
</script>
</body></html>""")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_timeout(1500)  # let the seeked frame paint
...
def main():
    ...
    key_timestamp_ms = parse_iso_to_ms(args.key_timestamp)

    events = build_events_for_timestamp(host, conn["project_id"], conn["api_key"], args.session_id, key_timestamp_ms)
    if not events:
        print(f"No rrweb events found for session {args.session_id}, skipping capture.")
        return 0

    out_path = "/tmp/capture.png"
    render_and_screenshot(events, key_timestamp_ms, out_path, vendor_dir)
    result = upload(args.worker_url, secret, args.session_id, args.task_index, conn["owner_email"], args.key_timestamp, out_path)
